...
- 排查思路：先看线程池活跃线程数、队列长度、任务等待时间、拒绝次数，再看线程状态和数据库连接池活跃连接数、等待时间、慢 SQL，区分是线程池排队、任务拆分不合理，还是下游资源被打满。  
- 参数设计：`corePoolSize` 和 `maxPoolSize` 不能只按 CPU 估，要结合任务是 IO 密集型还是 CPU 密集型，更要结合数据库连接池、下游 RPC 并发上限来定；阻塞队列建议用有界队列，避免无限堆积把 RT 隐藏掉。  
- 拒绝策略：`CallerRunsPolicy` 只适合可接受反压的场景，如果调用线程本身是 Web 请求线程，可能把入口线程拖慢；生产上更常见是快速失败、降级、告警或配合限流。  
- 线程很多但 CPU 不高：说明线程大概率卡在 IO、锁、数据库连接、网络返回等等待态；这时继续加线程只会增加排队、上下文切换和资源争抢，不会真正提升吞吐。  
- 保护下游：在线程池外增加限流、信号量、舱壁隔离，必要时通过 MQ 削峰，把瞬时并发改成可控速率；同时联动数据库连接池上限，避免线程池远大于下游承载能力。  
```
