...
## `volatile` 关键字的底层实现原理是什么？JVM 是通过什么机制，来保证它的可见性和有序性的？  
...
##  CAS（Compare-And-Swap）原理是什么？ 它在 Java 中如何实现的？  
 CAS（Compare-And-Swap）是一个原子操作，通过比较内存值和预期值来决定是否更新，它依赖 CPU 的原子指令保证不可分割性。在 Java 中，CAS 由 `Unsafe` 类封装，通过 CPU 指令实现，`AtomicInteger`、`AtomicReference` 等类就是在自旋循环中利用 CAS 实现线程安全的无锁原子更新。  
...
## `synchronized` 在 JVM 中是如何实现的？它、经历了哪些锁优化阶段？  
 ...
 每个对象都有一个对象头，在最初只有一个线程，没有竞争的时候 使用 CAS 将**线程 ID** 设置到对象的 Mark Word 头里 之后锁重入时 发现这个线程 ID是自己的就表示没有竞争，不用重新 CAS。 这是偏向锁,
...
1. 锁对象就是 Node
2. 不需要额外锁对象
3. JDK8 后 synchronized 已优化（偏向锁、轻量级锁）
4. 代码更简单
...
由于threalocal 是弱引用    GC 回收 ThreadLocal（key 变为 null）  
3️⃣ 但是 value 仍然被 ThreadLocalMap 强引用  
4️⃣ 如果线程长期存活（比如线程池线程）  
5️⃣ value 无法被回收    就会产生内存泄漏
...
+ 当 ThreadLocal 无外部强引用
+ GC 可回收 key
+ Entry.key = null
+ 下次 set/get 时会清理脏 Entry



那为什么 ThreadLocalMap 不在 key 为 null 时立即自动清理 value？

为什么还需要我们手动 remove？



 ThreadLocalMap 采用懒清理策略，只有在访问 ThreadLocalMap 时才清理 key 为 null 的 Entry。GC 只会回收弱引用 key，不会主动清理 value，因此在线程池等长生命周期线程中必须手动 remove。  
