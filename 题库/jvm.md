...
## RT 周期性抖高、CPU 不高、Full GC 增多、Old 区上下波动 怎么排查与优化

### 1）问题特征与第一判断

- 现象：RT 周期性抖高、CPU 不高、Full GC 增多、Old 区上下波动。
- 第一判断：优先怀疑 GC 节奏或对象分配模型问题，不先下结论为内存泄漏。
...
- `E`：Eden 使用率（快速打满说明分配速率高）
- `O`：Old 使用率（Full GC 后是否明显回落）
- `YGC/YGCT`：Young GC 次数/总耗时
- `FGC/FGCT`：Full GC 次数/总耗时（重点）
- `GCT`：总 GC 耗时

### 3）如何区分“短命对象多”还是“晋升压力大”

- 短命对象多：`YGC` 增长快、Eden 很快打满，Old 不一定持续升高。
- 晋升压力大：`FGC` 明显增长、Old 冲高快，日志中常见晋升失败/evacuation failure 信号。
- 面试表达：看对象是“死在年轻代”，还是“被迫提前进老年代”。

### 4）如果是短命对象多，优先怎么做

- 减少热点路径临时对象：循环里的 `new`、高频字符串拼接、重复 JSON 转换。
- 高频拼接改 `StringBuilder`，减少中间对象。
- 高频 `stream` 链路可改 `for` + 预分配容器，减少临时集合。
- 大对象一次性加载改分块/流式处理，降低瞬时分配洪峰。
- 复用缓冲区（如 `byte[]` / `ByteBuffer`），注意线程安全。

### 5）如果是晋升压力大，优先怎么做

- 目标：让对象尽量在年轻代被回收，减少被迫晋升。
- 适度放宽停顿目标，避免 GC 过度追求短停顿导致回收/晋升节奏失衡。
- 增加预留空间，降低高峰期复制/晋升失败概率。
- 提前触发并发标记，避免老年代“来不及回收”冲顶。
- 代码层减少超大对象与超大批次请求，拆批处理。
...
- 建议初始堆和最大堆尽量一致，减少运行时扩缩容抖动。

`-Xlog:gc*:file=gc.log:time,level,tags`（JDK11+）

- 打开 GC 证据日志，便于定位触发原因与停顿构成。
...
- 一次只改 1-2 个变量（参数或代码点），禁止“大杂烩改动”。
- 固定压测模型对比前后：`P95/P99`、`FGC/FGCT`、吞吐、错误率。
- 优化判定：RT 抖动下降 + Full GC 次数/停顿下降 + 吞吐不退化。
